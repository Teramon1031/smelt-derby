  ...
  return (
    <Animated.View
      style={{
        ...
        top: y,
        opacity: fadeAnim,
        transform: [{ translateX: swimAnim }, { scaleX: direction === 'right' ? -1 : 1 }],
      }}
    >
...
function BarRow({ participant, count, maxCount, rank, onTapFish, onLongPress }: BarRowProps) {
  ...
  return (
    <View style={barStyles.row}>
      <View style={barStyles.nameRow}>
        ...
        <Text style={[barStyles.name, isLeader && barStyles.leaderName]}>
          {participant.name}
        </Text>
      </View>
...
export default function DerbyScreen() {
  ...
  const sortedParticipants = useMemo(() => {
    ...
    return activeDerby.participants;
  }, [activeDerby]);

  const rankMap = useMemo(() => {
    if (!activeDerby) return {} as Record<string, number>;
    const counts = activeDerby.participants.map(p => ({ id: p.id, count: getCatchCount(p.id) }));
    counts.sort((a, b) => b.count - a.count);
    const map: Record<string, number> = {};
    counts.forEach((item, i) => { map[item.id] = i + 1; });
    return map;
  }, [activeDerby, getCatchCount]);
